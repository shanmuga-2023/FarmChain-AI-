...
// Displays real-time Order Placed & Order Status notifications
// ============================================
...
export function updateNotificationBellBadge() {
  ...
  document.querySelectorAll('.notification-btn, #notification-btn').forEach(btn => {
    ...
    if (count > 0) {
      ...
      btn.title = `${count} unread order notifications`;
    } else {
      if (dot) dot.remove();
      if (badge) badge.remove();
      btn.title = 'No new notifications';
    }
  });
}
  ...
  drawer.innerHTML = `
    <div style="padding: 16px; border-bottom: 1px solid var(--border-subtle); display: flex; align-items: center; justify-content: space-between; background: rgba(255,255,255,0.02);">
      <div style="display: flex; align-items: center; gap: 8px;">
        ...
        <span style="font-weight: 700; font-size: 0.95rem;">Order Notifications</span>
        ${unreadCount > 0 ? `<span class="badge badge-success" style="font-size: 0.72rem; padding: 2px 8px;">${unreadCount} New</span>` : ''}
      </div>
      <div style="display: flex; gap: 8px; align-items: center;">
        ${notifications.length > 0 ? `
          <button id="notif-mark-read-btn" style="background: none; border: none; color: var(--accent-green, #22c55e); font-size: 0.75rem; font-weight: 600; cursor: pointer;">✓ Read All</button>
          <button id="notif-clear-btn" style="background: none; border: none; color: var(--text-muted, #94a3b8); font-size: 0.75rem; cursor: pointer;">Clear</button>
        ` : ''}
        ...
        ">
          ...
          ${n.details ? `
            <div style="background: rgba(0,0,0,0.3); padding: 8px; border-radius: 6px; font-size: 0.75rem; color: var(--text-muted); display: flex; flex-direction: column; gap: 3px;">
              ${n.details.totalAmount ? `<div>💰 <strong>Amount:</strong> <span style="color: var(--accent-green);">${n.details.totalAmount}</span></div>` : ''}
              ${n.details.quantity ? `<div>📦 <strong>Quantity:</strong> ${n.details.quantity}</div>` : ''}
              ${n.details.escrowLocked ? `<div>⛓️ <strong>Escrow:</strong> ${n.details.escrowLocked}</div>` : ''}
            </div>
          ` : ''}
        </div>
      `).join('') : `
        <div style="text-align: center; padding: 40px 20px; color: var(--text-muted);">
          ...
          <div style="font-weight: 600; font-size: 0.9rem;">No Notifications Yet</div>
          <p style="font-size: 0.75rem; margin-top: 4px;">Order updates and status changes will appear here in real-time.</p>
        </div>
      `}
    </div>
  `;
